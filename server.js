...
app.use(express.json()); // Isso substitui o bodyParser.json()
...
app.post('/doar', (req, res) => {
    ...
    const query = 'INSERT INTO doacoes (nome, valor) VALUES (?, ?)';
    db.run(query, [nome, valor], function(err) {
        if (err) {
            console.error('Erro ao registrar doação:', err.message);
            return res.status(500).json({ error: 'Erro ao registrar doação.' });
        }
        res.json({ message: 'Doação registrada com sucesso!', id: this.lastID });
    });
});
